...
static std::string alicloud_proxy_host;
static int alicloud_proxy_port;
static std::string alicloud_proxy_user;
static std::string alicloud_proxy_passwd;

// List for all alicloud commands
static SimpleList<AlicloudGahpCommand*> alicloud_gahp_commands;

void set_alicloud_proxy_server(const char* url) 
{
	if( !url ) {
		return;
	}
	...
	if( !strncasecmp("http://", url, strlen("http://"))) {
		alicloud_proxy_host = url +  strlen("http://");
		alicloud_proxy_port = 80;
	}else if( !strncasecmp("https://", url, strlen("https://")) ) { 
		alicloud_proxy_host = url +  strlen("https://");
		alicloud_proxy_port = 443;
	}else {
		alicloud_proxy_host = url;
		alicloud_proxy_port = 80;
	}
	
	/* sateesh added code to even handle proxy username and password */
	/* This code cannot handle passwords containing @ ? */
	/* Exact format of ALICLOUD_HTTP_PROXY is -- http://userid:password@host:port */
    size_t pos = alicloud_proxy_host.find('@');
	if( std::string::npos != pos ) {
	  alicloud_proxy_user = alicloud_proxy_host.substr(0, pos);
	  
	  alicloud_proxy_host = alicloud_proxy_host.substr(pos + 1,
		  alicloud_proxy_host.length());

      pos = alicloud_proxy_user.find(':');
	  if( std::string::npos != pos ) {
		alicloud_proxy_passwd = alicloud_proxy_user.substr(pos + 1,
			alicloud_proxy_user.length());
		alicloud_proxy_user = alicloud_proxy_user.substr(0, pos);
	  }
	}

    pos = alicloud_proxy_host.find(':');
	if( std::string::npos != pos ) {
		int port =
			atoi(alicloud_proxy_host.substr(pos + 1,
										  alicloud_proxy_host.length()).c_str());

		if( port > 0 ) {
			alicloud_proxy_port = port;
		}

		alicloud_proxy_host = alicloud_proxy_host.substr(0, pos);
	}
         
	dprintf(D_ALWAYS, "Using proxy server, host=%s, port=%d user=%s\n", 
		alicloud_proxy_host.c_str(), alicloud_proxy_port, 
		alicloud_proxy_user.c_str());
}

bool get_alicloud_proxy_server(const char* &host_name, int& port, const char* &user_name, const char* &passwd )
{
	if( alicloud_proxy_host.empty() == false ) {
		host_name = alicloud_proxy_host.c_str();
		port = alicloud_proxy_port;
		user_name = alicloud_proxy_user.c_str();
		passwd = alicloud_proxy_passwd.c_str();
		return true;
	}
...
AlicloudGahpCommand::AlicloudGahpCommand(const char* cmd, ioCheckfn iofunc, workerfn workerfunc)
{
	command = cmd;
...
void
registerAlicloudGahpCommand(const char* command, ioCheckfn iofunc, workerfn workerfunc)
{
	if( !command ) {
		return;
	}

	AlicloudGahpCommand* newcommand = new AlicloudGahpCommand(command, iofunc, workerfunc);
	ASSERT(newcommand);

	alicloud_gahp_commands.Append(newcommand);
}

int
numofAlicloudCommands(void)
{
	return alicloud_gahp_commands.Number();
}

int 
allAlicloudCommands(StringList &output)
{
	AlicloudGahpCommand *one_cmd = NULL;

	alicloud_gahp_commands.Rewind();
	while( alicloud_gahp_commands.Next(one_cmd) ) {
...
create_success_result( int req_id, StringList *result_list)
{
	...
	tmp_result[0] = ALICLOUD_COMMAND_SUCCESS_OUTPUT;
...
create_failure_result( int req_id, const char *err_msg, const char* err_code)
{
	...
	tmp_result[0] = ALICLOUD_COMMAND_ERROR_OUTPUT;
