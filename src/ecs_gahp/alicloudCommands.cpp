...
// except for by AlicloudQuery::SendRequest().
//
std::string alicloudURLEncode( const std::string & input )
{
    /*
     * See http://docs.alicloudwebservices.com/ALIEC2/2010-11-15/DeveloperGuide/using-query-api.html
     *
     *
    ...
    for( unsigned i = 0; i < input.length(); ++i ) {
        ...
        if( ('A' <= input[i] && input[i] <= 'Z')
         ...
         || input[i] == '~') {
            ...
            int written = snprintf( percentEncode, 4, "%%%02X", v );
            ASSERT( written == 3 );
...
AlicloudRequest::~AlicloudRequest() { }
...
class Throttle {
    ...
    public:
        // Managed by AlicloudRequest::SendRequest() because the param system
...
bool AlicloudRequest::SendRequest() {
    query_parameters.insert( std::make_pair( "Version", "2014-05-26" ) );
...
std::string AlicloudRequest::canonicalizeQueryString() {
    ...
    for( auto i = query_parameters.begin(); i != query_parameters.end(); ++i ) {
        ...
        // and then compose the string out of that, in case alicloudURLEncode()
        // changes the sort order, but we don't specify parameters like that.

        // Step 1B: Encode the parameter names and values.
        std::string name = alicloudURLEncode( i->first );
        std::string value = alicloudURLEncode( i->second );
...
bool doSha1(    const std::string & payload,
...
std::string pathEncode( const std::string & original ) {
    ...
    while( offset < length ) {
        ...
        encoded += alicloudURLEncode( segment );
...
bool AlicloudRequest::createV4Signature(    const std::string & payload,
    ...
    // S3 complains if x-amz-content-sha1 isn't signed, which makes sense,
    // so do this early.

    // The canonical payload hash is the lowercase hexadecimal string of the
    // (SHA1) hash value of the payload.
    unsigned int mdLength = 0;
    unsigned char messageDigest[EVP_MAX_MD_SIZE];
    if(! doSha1( payload, messageDigest, & mdLength )) {
        this->errorCode = "E_INTERNAL";
    ...
    if( sendContentSHA ) {
        headers[ "x-amz-content-sha1" ] = payloadHash;
    }
    ...
    AlicloudRequest::AttributeValueMap transformedHeaders;
    ...
    if(! doSha1( canonicalRequest, messageDigest, & mdLength )) {
        this->errorCode = "E_INTERNAL";
    ...
    formatstr( stringToSign, "ALI4-HMAC-SHA1\n%s\n%s\n%s",
    ...
    hmac = HMAC( EVP_sha1(), messageDigest, mdLength,
        (const unsigned char *)r.c_str(), r.length(), messageDigest2, & md2Length );
    if( hmac == NULL ) { return false; }

    hmac = HMAC( EVP_sha1(), messageDigest2, md2Length,
    ...
    formatstr( authorizationValue, "ALI4-HMAC-SHA1 Credential=%s/%s,"
...
bool AlicloudRequest::sendV4Request( const std::string & payload, bool sendContentSHA ) {
...
bool AlicloudRequest::sendV2Request() {
    ...
    //
    //      Action, Version, AccessKeyId, Timestamp (or Expires),
    ...
    //
    // The AccessKeyId is just the contents of this->accessKeyFile,
    ...
    //query_parameters.insert( std::make_pair( "SignatureMethod", "HmacSHA1" ) );
     ...
     * the signature.  See
     *
     * http://docs.alicloudwebservices.com/ALIEC2/2010-11-15/DeveloperGuide/using-query-api.html
     *
     */
    ...
    // you just created, your Secret Access Key as the key, and SHA1
    ...
    if( protocol == "x509" ) {
        ...
        // since the x.509 implementation ignores the AccessKeyId
...
//std::string Signature = Base64(HMAC-SHA1(UTF-8-Encoding-Of( saKey, stringToSign ) ) ); 
    ...
    canonicalQueryString += "&Signature=" +  alicloudURLEncode( signatureInBase64 );
    ...
    // The ALI docs now say that " " - > "%20", and that "+" is an error.
    ...
    //canonicalQueryString =  "https://ecs.aliyuncs.com/?"+canonicalQueryString;
    return sendPreparedRequest( protocol, postURI, canonicalQueryString );
}

bool AlicloudRequest::SendURIRequest() {
    httpVerb = "POST";
    std::string noPayloadAllowed;
    return sendV4Request( noPayloadAllowed );
}

bool AlicloudRequest::SendJSONRequest( const std::string & payload ) {
...
bool AlicloudRequest::SendS3Request( const std::string & payload ) {
...
bool AlicloudRequest::sendPreparedRequest(
...
*/
    ...
    //rv = curl_easy_setopt( curl, CURLOPT_HTTPGET,  & payload );
...
AlicloudVMStart::~AlicloudVMStart() { }
...
void vmStartESH( void * vUserData, const XML_Char * name, const XML_Char ** ) {
    ...
    if( strcasecmp( ignoringNameSpace( name ), "InstanceIdSet" ) == 0 ) {
...
bool AlicloudVMStart::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if ( result ) {
...
bool AlicloudVMStart::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_START" ) == 0 );

    // Uses the Query API function 'RunInstances', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-RunInstances.html
    int requestID;
    ...
    AlicloudVMStart vmStartRequest = AlicloudVMStart( requestID, argv[0] );
    ...
    fp = fopen("/usr/local/condor/config.txt","r");
    while(!feof(fp))
    {
        //regionid[txtlong++] = fgetc(fp);
        regionidstr += fgetc(fp);
    }
    regionid[txtlong] = 0;
    fclose(fp);*/
    ...
    std::string line; 
    getline (in, line);
    vmStartRequest.query_parameters[ "RegionId" ] = line;
    //vmStartRequest.query_parameters[ "RegionId" ] = REGIONID;
    vmStartRequest.query_parameters[ "VSwitchId" ] = argv[12];
    ...
    vmStartRequest.query_parameters[ "Amount" ] = "1";
    //vmStartRequest.query_parameters[ "SecurityGroupId" ] = argv[13];
    ...
    if( strcasecmp( argv[17], NULLSTRING ) ) {
        vmStartRequest.query_parameters[ "Amount" ] = argv[17];
    }

    if( strcasecmp( argv[19], NULLSTRING ) ) {
        //!vmStartRequest.query_parameters[ "MaxCount" ] = argv[17];
        vmStartRequest.query_parameters[ "SecurityGroupId" ] = argv[19];
    }
    //vmStartRequest.query_parameters[ "SecurityGroupId" ] = "sg-2ze7skefrz52k3caqhzd";

    if( strcasecmp( argv[11], NULLSTRING ) ) {
        //!vmStartRequest.query_parameters[ "MaxCount" ] = argv[17];
        vmStartRequest.query_parameters[ "VSwitchId" ] = argv[11];
    }
...
AlicloudVMStartSpot::~AlicloudVMStartSpot() { }
...
bool AlicloudVMStartSpot::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if ( result ) {
...
bool AlicloudVMStartSpot::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], "EC2_VM_START_SPOT" ) == 0 );

    // Uses the Query AP function 'RequestSpotInstances', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-RequestSpotInstances.html
    ...
    AlicloudVMStartSpot vmSpotRequest = AlicloudVMStartSpot( requestID, argv[0] );
    ...
    return true;
} // end AlicloudVMStartSpot::workerFunction()

// ---------------------------------------------------------------------------

AlicloudVMStop::~AlicloudVMStop() { }

// Expecting:EC2_VM_STOP <req_id> <serviceurl> <accesskeyfile> <secretkeyfile> <instance-id>
bool AlicloudVMStop::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_STOP" ) == 0 );

    // Uses the Query API function 'TerminateInstances', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-TerminateInstances.html
    ...
    // Fill in required attributes & parameters.
    AlicloudVMStop terminationRequest = AlicloudVMStop( requestID, argv[0] );
    ...
    terminationRequest.query_parameters[ "Action" ] = "DeleteInstance";
    terminationRequest.query_parameters[ "InstanceId" ] = argv[5];
    terminationRequest.query_parameters[ "Force" ] = "true";
    ...
    if( ! terminationRequest.SendRequest() ) {
        ...
        // AlicloudVMStop::SendRequest() could parse its resultString
...
AlicloudVMStopSpot::~AlicloudVMStopSpot() { }

bool AlicloudVMStopSpot::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], "EC2_VM_STOP_SPOT" ) == 0 );

    // Uses the Query API function 'CancelSpotInstanceRequests', documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-CancelSpotInstanceRequests.html
    ...
    AlicloudVMStopSpot terminationRequest = AlicloudVMStopSpot( requestID, argv[0] );
    terminationRequest.serviceURL = argv[2];
    terminationRequest.accessKeyFile = argv[3];
    terminationRequest.secretKeyFile = argv[4];
    ...
    return true;
} // end AlicloudVMStop::workerFunction()

// ---------------------------------------------------------------------------

AlicloudVMStatus::~AlicloudVMStatus() { }

// Expecting:EC2_VM_STATUS <req_id> <serviceurl> <accesskeyfile> <secretkeyfile> <instance-id>
bool AlicloudVMStatus::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_STATUS" ) == 0 );

    // Uses the Query API function 'DescribeInstances', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-DescribeInstances.html
    ...
    // Fill in required attributes & parameters.
    AlicloudVMStatus sRequest = AlicloudVMStatus( requestID, argv[0] );
    ...
    //sRequest.query_parameters[ "RegionId" ] = REGIONID;
    ...
    //! sRequest.query_parameters[ "InstanceIds" ] = argv[5];
    ...
    if( ! sRequest.SendRequest() ) {
        ...
        if( sRequest.results.size() == 0 ) {
            ...
            for( unsigned i = 0; i < sRequest.results.size(); ++i ) {
                AlicloudStatusResult asr = sRequest.results[i];
		...
		resultList.append( asr.status.c_str() );
                ...
                if( strcasecmp( asr.status.c_str(), ALICLOUD_STATUS_RUNNING ) == 0 ) {
...
AlicloudVMStatusSpot::~AlicloudVMStatusSpot() { }
...
struct vmStatusSpotUD_t {
    ...
    AlicloudStatusSpotResult * currentResult;

    std::vector< AlicloudStatusSpotResult > & results;

    vmStatusSpotUD_t( std::vector< AlicloudStatusSpotResult > & assrList ) :
        inItem( 0 ),
        inStatus( false ),
...
void vmStatusSpotESH( void * vUserData, const XML_Char * name, const XML_Char ** ) {
    ...
    if( strcasecmp( ignoringNameSpace( name ), "item" ) == 0 ) {
        if( vsud->inItem == 0 ) {
            vsud->currentResult = new AlicloudStatusSpotResult();
...
bool AlicloudVMStatusSpot::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
    ...
    return result;
} // end AlicloudVMStatusSpot::SendRequest()

bool AlicloudVMStatusSpot::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_STATUS_SPOT" ) == 0 );

    // Uses the Query API function 'DescribeSpotInstanceRequests', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-DescribeSpotInstanceRequests.html
    ...
    AlicloudVMStatusSpot statusRequest = AlicloudVMStatusSpot( requestID, argv[0] );
    ...
    if( ! statusRequest.SendRequest() ) {
        ...
        if( statusRequest.spotResults.size() == 0 ) {
            ...
            for( unsigned i = 0; i < statusRequest.spotResults.size(); ++i ) {
                AlicloudStatusSpotResult & assr = statusRequest.spotResults[i];
    ...
    return true;
} // end AlicloudVmStatusSpot::workerFunction()

// ---------------------------------------------------------------------------

AlicloudVMStatusAllSpot::~AlicloudVMStatusAllSpot( ) { }

bool AlicloudVMStatusAllSpot::workerFunction(char **argv, int argc, std::string &result_string) {
    ...
    AlicloudVMStatusAllSpot statusRequest = AlicloudVMStatusAllSpot( requestID, argv[0] );
    statusRequest.serviceURL = argv[2];
    statusRequest.accessKeyFile = argv[3];
    statusRequest.secretKeyFile = argv[4];
    statusRequest.query_parameters[ "Action" ] = "DescribeSpotInstanceRequests";
    ...
    return true;
} // end AlicloudVmStatusAllSpot::workerFunction()

// ---------------------------------------------------------------------------

AlicloudVMStatusAll::~AlicloudVMStatusAll() { }
...
struct vmStatusUD_t {
    ...
    AlicloudStatusResult * currentResult;
    ...
    vmStatusUD_t( std::vector< AlicloudStatusResult > & asrList ) :
...
void vmStatusESH( void * vUserData, const XML_Char * name, const XML_Char ** ) {
    ...
    if( ! vsud->inInstancesSet ) {
        //if( strcasecmp( ignoringNameSpace( name ), "Instances" ) == 0 ) {
        if( strcasecmp( ignoringNameSpace( name ), "Instances" ) == 0 ) {
    ...
    if( ! vsud->inInstance ) {
        if( strcasecmp( ignoringNameSpace( name ), "Instance" ) == 0 ) {
            vsud->currentResult = new AlicloudStatusResult();
            assert( vsud->currentResult != NULL );
    ...
    if( strcasecmp( ignoringNameSpace( name ), "SecurityGroupIds" ) == 0 ) {
    ...
    //
    if( vsud->inGroupSet ) {
        if( strcasecmp( ignoringNameSpace( name ), "SecurityGroupId" ) == 0 ) {
            vsud->inGroup = true;
            return;
        }
    }
    ...
    else if( strcasecmp( ignoringNameSpace( name ), "KeyPairName" ) == 0 ) {
...
void vmStatusEEH( void * vUserData, const XML_Char * name ) {
    ...
    /*if( strcasecmp( ignoringNameSpace( name ), "InstanceType" ) == 0 ) {
        ASSERT( vsud->inInstanceState );
        vsud->inInstanceState = false;
        return;
    }*/

    /*if( strcasecmp( ignoringNameSpace( name ), "Status" ) == 0 ) {
        ASSERT( vsud->inStateReason );
        vsud->inStateReason = false;
        return;
    }*/

    /*if( strcasecmp( ignoringNameSpace( name ), "Tags" ) == 0 ) {
        vsud->inTagSet = false;
        return;
    }
    if( vsud->inTag && strcasecmp( ignoringNameSpace( name ), "TagKey" ) == 0 ) {
        vsud->inWhichTag = vmStatusUD::NONE;
        return;
    }
    if( vsud->inTag && strcasecmp( ignoringNameSpace( name ), "TagValue" ) == 0 ) {
        vsud->inWhichTag = vmStatusUD::NONE;
        return;
    }*/
}

bool AlicloudVMStatusAll::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
...
bool AlicloudVMStatusAll::workerFunction(char **argv, int argc, std::string &result_string) {
    ...
    // Fill in required attributes & parameters.
    AlicloudVMStatusAll saRequest = AlicloudVMStatusAll( requestID, argv[0] );
    ...
    std::string line; 
    getline (in, line);
	saRequest.query_parameters[ "RegionId" ] = line;

    //saRequest.query_parameters[ "RegionId" ] = REGIONID;
    ...
    if( ! saRequest.SendRequest() ) {
        ...
        if( saRequest.results.size() == 0 ) {
            ...
            for( unsigned i = 0; i < saRequest.results.size(); ++i ) {
                AlicloudStatusResult & asr = saRequest.results[i];
                resultList.append( asr.instance_id.c_str() );
                transform(asr.status.begin(), asr.status.end(), asr.status.begin(), ::tolower);  
                resultList.append( asr.status.c_str() );
                // For GT #3682: only one of the following two may be null.
                resultList.append( nullStringIfEmpty( asr.clientToken ) );
                resultList.append( nullStringIfEmpty( asr.keyname ) );
                //resultList.append( nullStringIfEmpty( "key-01" ) );
                resultList.append( nullStringIfEmpty( asr.stateReasonCode ) );
                resultList.append( nullStringIfEmpty( asr.public_dns ) );
...
AlicloudVMCreateKeypair::~AlicloudVMCreateKeypair() { }

struct privateKeyUD_t {
    bool inKeyMaterial;
    std::string PrivateKeyBody;
...
void createKeypairESH( void * vUserData, const XML_Char * name, const XML_Char ** ) {
    ...
    //! if( strcasecmp( ignoringNameSpace( name ), "PrivateKeyBody" ) == 0 ) {
    if( strcasecmp( ignoringNameSpace( name ), "PrivateKeyBody" ) == 0 ) {
...
void createKeypairCDH( void * vUserData, const XML_Char * cdata, int len ) {
    ...
    if( pkud->inKeyMaterial ) {
        appendToString( (const void *)cdata, len, 1, (void *) & pkud->PrivateKeyBody );
    }
}
...
bool AlicloudVMCreateKeypair::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
        privateKeyUD pkud;
        XML_Parser xp = XML_ParserCreate( NULL );
        ...
        if( ! writeShortFile( this->privateKeyFileName, pkud.PrivateKeyBody ) ) {
            this->errorCode = "E_FILE_IO";
...
bool AlicloudVMCreateKeypair::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_CREATE_KEYPAIR" ) == 0 );

    // Uses the Query API function 'CreateKeyPair', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-CreateKeyPair.html
    ...
    // Fill in required attributes & parameters.
    AlicloudVMCreateKeypair ckpRequest = AlicloudVMCreateKeypair( requestID, argv[0] );
    ...
    ckpRequest.query_parameters[ "KeyPairName" ] = argv[5];
    ...
    std::string line; 
    getline (in, line);
	ckpRequest.query_parameters[ "RegionId" ] = line;

    //ckpRequest.query_parameters[ "RegionId" ] = REGIONID;
...
AlicloudVMDestroyKeypair::~AlicloudVMDestroyKeypair() { }

// Expecting:EC2_VM_DESTROY_KEYPAIR <req_id> <serviceurl> <accesskeyfile> <secretkeyfile> <keyname>
bool AlicloudVMDestroyKeypair::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_DESTROY_KEYPAIR" ) == 0 );

    // Uses the Query API function 'DeleteKeyPair', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-DeleteKeyPair.html
    ...
    // Fill in required attributes & parameters.
    AlicloudVMDestroyKeypair dkpRequest = AlicloudVMDestroyKeypair( requestID, argv[0] );
    dkpRequest.serviceURL = argv[2];
    dkpRequest.accessKeyFile = argv[3];
    dkpRequest.secretKeyFile = argv[4];
    dkpRequest.query_parameters[ "Action" ] = "DeleteKeyPairs";
    dkpRequest.query_parameters[ "KeyPairNames" ] = argv[5];
    dkpRequest.query_parameters[ "KeyPairNames" ] = "[\""+dkpRequest.query_parameters[ "KeyPairNames" ]+"\"]";
    //dkpRequest.query_parameters[ "RegionId" ] = REGIONID;
    ...
    std::string line; 
    getline (in, line);
	dkpRequest.query_parameters[ "RegionId" ] = line;
...
AlicloudAssociateAddress::~AlicloudAssociateAddress() { }

// Expecting:EC2_VM_ASSOCIATE_ADDRESS <req_id> <serviceurl> <accesskeyfile> <secretkeyfile> <instance-id> <elastic-ip>
bool AlicloudAssociateAddress::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_ASSOCIATE_ADDRESS" ) == 0 );

    // Uses the Query API function 'AssociateAddress', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/index.html?ApiReference-query-AssociateAddress.html
    ...
    // Fill in required attributes & parameters.
    AlicloudAssociateAddress asRequest = AlicloudAssociateAddress( requestID, argv[0] );
    ...
    asRequest.query_parameters[ "Action" ] = "AssociateEipAddress";
	...
	getline(f, line);
	fclose(f);
    asRequest.query_parameters[ "RegionId" ] = REGIONID;*/
    std::ifstream in("/usr/local/condor/config.txt");  
    std::string line; 
    getline (in, line);
    asRequest.query_parameters[ "RegionId" ] = line;
...
bool
AlicloudCreateTags::workerFunction(char **argv,
                                 int argc,
                                 std::string &result_string)
{
    ...
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/index.html?ApiReference-query-CreateTags.html
    ...
    // Fill in required attributes & parameters.
    AlicloudCreateTags asRequest = AlicloudCreateTags( requestID, argv[0] );
    asRequest.serviceURL = argv[2];
    asRequest.accessKeyFile = argv[3];
    asRequest.secretKeyFile = argv[4];
    asRequest.query_parameters["Action"] = "AddTags";
    asRequest.query_parameters["ResourceId"] = argv[5];
    asRequest.query_parameters["ResourceType"] = "instance";
...
AlicloudAttachVolume::~AlicloudAttachVolume() { }

bool AlicloudAttachVolume::workerFunction(char **argv, int argc, std::string &result_string) 
{
    ...
    // Fill in required attributes & parameters.
    AlicloudAttachVolume asRequest = AlicloudAttachVolume( requestID, argv[0] );
    asRequest.serviceURL = argv[2];
    asRequest.accessKeyFile = argv[3];
    asRequest.secretKeyFile = argv[4];
...
// The types detected are Alicloud, OpenStack, Nimbus, and Eucalyptus.
// If the server's response doesn't match any of these types, then
// "Unknown" is returned.
// The following checks are performed:
// * Alicloud's header includes "Server: AlicloudEC2"
...
// * Alicloud and Nimbus's <?xml?> tag includes version="1.0" and
...
AlicloudVMServerType::~AlicloudVMServerType() { }

bool AlicloudVMServerType::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
        serverType = "Unknown";
        ...
        if ( header.find( "RegionId" ) != std::string::npos ) {
            server_alicloud = true;
        }
        ...
        serverType = "Amazon";
        if ( server_alicloud && !server_jetty && xml_tag &&
             xml_encoding && request_id && !euca_tag ) {
            serverType = "Alicloud";
        } else if ( !server_alicloud && !server_jetty && xml_tag &&
                    !xml_encoding && request_id && !euca_tag ) {
...
bool AlicloudVMServerType::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcmp( argv[0], "EC2_VM_SERVER_TYPE" ) == 0 );

    // Uses the Query API function 'DescribeKeyPairs', as documented at
    // http://docs.alicloudwebservices.com/ALIEC2/latest/APIReference/ApiReference-query-DescribeKeyPairs.html
    ...
    // Fill in required attributes & parameters.
    AlicloudVMServerType serverTypeRequest = AlicloudVMServerType( requestID, argv[0] );
	...
	getline(f, line);
	fclose(f);
    serverTypeRequest.query_parameters[ "RegionId" ] = REGIONID;*/
    std::ifstream in("/usr/local/condor/config.txt");  
    std::string line; 
    getline (in, line);
	serverTypeRequest.query_parameters[ "RegionId" ] = line;
...
AlicloudBulkStart::~AlicloudBulkStart() { }
...
bool AlicloudBulkStart::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
...
void AlicloudBulkStart::setLaunchSpecificationAttribute(
...
bool AlicloudBulkStart::workerFunction( char ** argv, int argc, std::string & result_string ) {
    ...
    // Fill in required attributes.
    AlicloudBulkStart request = AlicloudBulkStart( requestID, argv[0] );
    ...
    request.query_parameters[ "Version" ] = "2014-05-26";
    ...
    for( int i = 11; i < argc; ++i ) {
        ...
        request.setLaunchSpecificationAttribute( lcIndex, blob,
            "KeyName" );
        // ALI' documentation is wrong, claims this is 'UserData'.
        request.setLaunchSpecificationAttribute( lcIndex, blob,
            "userData", "UserData" );
        ...
        if(! blob[ "SecurityGroupNames" ].empty()) {
            ...
            for( unsigned i = 0; (groupName = sl.next()) != NULL; ++i ) {
                ...
                ss << "SpotFleetRequestConfig.LaunchSpecifications.";
                ss << lcIndex << ".";
                // ALI' documentation is wrong, claims this is 'SecurityGroups'.
        ...
        if(! blob[ "BlockDeviceMapping" ].empty() ) {
            ...
            for( unsigned i = 0; (mapping = sl.next()) != NULL; ++i ) {
                ...
                ss << "SpotFleetRequestConfig.LaunchSpecifications.";
                ss << lcIndex << ".";
                // ALI' documentation is wrong, claims this is plural.
                ss << "BlockDeviceMapping." << i;

                if( strchr( mapping, '=' ) != NULL ) {
                    // New-style mapping (copied from ALI web console).
...
AlicloudPutRule::~AlicloudPutRule() { }

bool AlicloudPutRule::SendJSONRequest( const std::string & payload ) {
    bool result = AlicloudRequest::SendJSONRequest( payload );
    if( result ) {
...
bool AlicloudPutRule::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], ALICLOUD_COMMAND_PUT_RULE ) == 0 );
    int requestID;
    get_int( argv[1], & requestID );
    dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
        requestID, argv[0] );
    ...
    // Fill in required attributes.
    AlicloudPutRule request = AlicloudPutRule( requestID, argv[0] );
    request.serviceURL = argv[2];
    request.accessKeyFile = argv[3];
    request.secretKeyFile = argv[4];

    // Set the required headers.  (SendJSONRequest() sets the content-type.)
    request.headers[ "X-Amz-Target" ] = "ALIEvents.PutRule";
...
AlicloudPutTargets::~AlicloudPutTargets() { }

bool AlicloudPutTargets::SendJSONRequest( const std::string & payload ) {
    bool result = AlicloudRequest::SendJSONRequest( payload );
    if( result ) {
        // We could use a dedicated JSON parser, but its API is annoying,
        // and the ClassAd converter is good enough.
...
bool AlicloudPutTargets::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], ALICLOUD_COMMAND_PUT_TARGETS ) == 0 );
    int requestID;
    get_int( argv[1], & requestID );
    dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
        requestID, argv[0] );
    ...
    // Fill in required attributes.
    AlicloudPutTargets request = AlicloudPutTargets( requestID, argv[0] );
    request.serviceURL = argv[2];
    request.accessKeyFile = argv[3];
    request.secretKeyFile = argv[4];

    // Set the required headers.  (SendJSONRequest() sets the content-type.)
    request.headers[ "X-Amz-Target" ] = "ALIEvents.PutTargets";
...
AlicloudRemoveTargets::~AlicloudRemoveTargets() { }

bool AlicloudRemoveTargets::SendJSONRequest( const std::string & payload ) {
    bool result = AlicloudRequest::SendJSONRequest( payload );
    if( result ) {
        ClassAd reply;
        classad::ClassAdJsonParser cajp;
...
bool AlicloudRemoveTargets::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], ALICLOUD_COMMAND_REMOVE_TARGETS ) == 0 );
    int requestID;
    get_int( argv[1], & requestID );
    dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
        requestID, argv[0] );
    ...
    // Fill in required attributes.
    AlicloudRemoveTargets request = AlicloudRemoveTargets( requestID, argv[0] );
    request.serviceURL = argv[2];
    request.accessKeyFile = argv[3];
    request.secretKeyFile = argv[4];

    // Set the required headers.  (SendJSONRequest() sets the content-type.)
    request.headers[ "X-Amz-Target" ] = "ALIEvents.RemoveTargets";
...
AlicloudDeleteRule::~AlicloudDeleteRule() { }

bool AlicloudDeleteRule::SendJSONRequest( const std::string & payload ) {
    bool result = AlicloudRequest::SendJSONRequest( payload );
    // Succesful results are defined to be empty.
    return result;
}

bool AlicloudDeleteRule::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], ALICLOUD_COMMAND_DELETE_RULE ) == 0 );
    int requestID;
    get_int( argv[1], & requestID );
    dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
        requestID, argv[0] );
    ...
    // Fill in required attributes.
    AlicloudDeleteRule request = AlicloudDeleteRule( requestID, argv[0] );
    request.serviceURL = argv[2];
    request.accessKeyFile = argv[3];
    request.secretKeyFile = argv[4];

    // Set the required headers.  (SendJSONRequest() sets the content-type.)
    request.headers[ "X-Amz-Target" ] = "ALIEvents.DeleteRule";
...
AlicloudBulkStop::~AlicloudBulkStop() { }
...
bool AlicloudBulkStop::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
...
bool AlicloudBulkStop::workerFunction( char ** argv, int argc, std::string & result_string ) {
    ...
    // Fill in required attributes.
    AlicloudBulkStop request = AlicloudBulkStop( requestID, argv[0] );
    request.serviceURL = argv[2];
    request.accessKeyFile = argv[3];
    request.secretKeyFile = argv[4];
    ...
    // See comment in AlicloudBulkStart::workerFunction().
    request.query_parameters[ "Version" ] = "2014-05-26";
...
AlicloudGetFunction::~AlicloudGetFunction() { }

bool AlicloudGetFunction::SendURIRequest() {
    bool result = AlicloudRequest::SendURIRequest();
    if( result ) {
        ClassAd reply;
        classad::ClassAdJsonParser cajp;
        if(! cajp.ParseClassAd( this->resultString, reply, true )) {
        ...
        if(! configuration.LookupString( "CodeSha1", functionHash )) {
            dprintf( D_ALWAYS, "Failed to find string value 'Configuration.CodeSha1' in reply.\n" );
            return false;
        }
...
bool AlicloudGetFunction::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], ALICLOUD_COMMAND_GET_FUNCTION ) == 0 );
    int requestID;
    get_int( argv[1], & requestID );
    dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
        requestID, argv[0] );
    ...
    // Fill in required attributes.
    AlicloudGetFunction request = AlicloudGetFunction( requestID, argv[0] );
    request.accessKeyFile = argv[3];
    request.secretKeyFile = argv[4];
    ...
    formatstr( request.serviceURL, "%s%s2015-03-31/functions/%s", argv[2],
        separator, alicloudURLEncode( argv[5] ).c_str() );
...
AlicloudS3Upload::~AlicloudS3Upload() { }

bool AlicloudS3Upload::SendRequest() {
    httpVerb = "PUT";
    std::string payload;
...
bool AlicloudS3Upload::workerFunction(char **argv, int argc, std::string &result_string) {
    ...
    // Fill in required attributes & parameters.
    AlicloudS3Upload uploadRequest = AlicloudS3Upload( requestID, argv[0] );
...
AlicloudCreateStack::~AlicloudCreateStack() { }

bool AlicloudCreateStack::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
        Regex r; int errCode = 0; const char * errString = 0;
...
bool AlicloudCreateStack::workerFunction(char **argv, int argc, std::string &result_string) {
    ...
    // Fill in required attributes & parameters.
    AlicloudCreateStack csRequest = AlicloudCreateStack( requestID, argv[0] );
    ...
    csRequest.query_parameters[ "Version" ] = "2014-05-26";
...
AlicloudDescribeStacks::~AlicloudDescribeStacks() { }

bool AlicloudDescribeStacks::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
...
//              <serviceurl> <accesskeyfile> <secretkeyfile>
//              <stackName>
bool AlicloudDescribeStacks::workerFunction(char **argv, int argc, std::string &result_string) {
    ...
    // Fill in required attributes & parameters.
    AlicloudDescribeStacks dsRequest = AlicloudDescribeStacks( requestID, argv[0] );
    ...
    dsRequest.query_parameters[ "Version" ] = "2014-05-26";
...
AlicloudCallFunction::~AlicloudCallFunction() { }

bool AlicloudCallFunction::SendJSONRequest( const std::string & payload ) {
    bool result = AlicloudRequest::SendJSONRequest( payload );
    return result;
}
...
bool AlicloudCallFunction::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], ALICLOUD_COMMAND_CALL_FUNCTION ) == 0 );
    int requestID;
    get_int( argv[1], & requestID );
    dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
        requestID, argv[0] );
    ...
    // Fill in required attributes.
    AlicloudCallFunction request = AlicloudCallFunction( requestID, argv[0] );
    request.accessKeyFile = argv[3];
    request.secretKeyFile = argv[4];
...
AlicloudBulkQuery::~AlicloudBulkQuery() { }
...
bool AlicloudBulkQuery::SendRequest() {
    bool result = AlicloudRequest::SendRequest();
    if( result ) {
...
bool AlicloudBulkQuery::workerFunction( char ** argv, int argc, std::string & result_string ) {
    ...
    // Fill in required attributes.
    AlicloudBulkQuery request = AlicloudBulkQuery( requestID, argv[0] );
    request.serviceURL = argv[2];
    request.accessKeyFile = argv[3];
    request.secretKeyFile = argv[4];
