...
#define ECS_VM_STATE_RUNNING			"running"
#define ECS_VM_STATE_PENDING			"pending"
#define ECS_VM_STATE_SHUTTINGDOWN		"shutting-down"
#define ECS_VM_STATE_TERMINATED			"terminated"
#define ECS_VM_STATE_SHUTOFF			"shutoff"
#define ECS_VM_STATE_STOPPED			"stopped"

// These are pseduostates used internally by the grid manager.  It's
// important that they aren't returned by the ECS API, since that's how
// we block until the next status poll has occured and not deadlock if
// the job has already reached a terminal state.
#define ECS_VM_STATE_PURGED				"not-found"
#define ECS_VM_STATE_CANCELLING			"cancelling"
#define ECS_VM_STATE_SPOT_CANCELLING	"cancelling-spot"
...
void ECSJobReconfig()
{
	...
	ECSJob::setGahpCallTimeout( gct );

	int cfrc = param_integer("GRIDMANAGER_CONNECT_FAILURE_RETRY_COUNT", 3);
	ECSJob::setConnectFailureRetry( cfrc );

	// Tell all the resource objects to deal with their new config values
	ECSResource *next_resource;

	ECSResource::ResourcesByName.startIterations();

	while ( ECSResource::ResourcesByName.iterate( next_resource ) != 0 ) {
		next_resource->Reconfig();
	}
}


bool ECSJobAdMatch( const ClassAd *job_ad )
{
	int universe;
	string resource;
	...
	if ( (universe == CONDOR_UNIVERSE_GRID) &&
		 (strncasecmp( resource.c_str(), "ecs", 3 ) == 0) )
	{
		return true;
	}
	return false;
...
BaseJob* ECSJobCreate( ClassAd *jobad )
{
	return (BaseJob *)new ECSJob( jobad );
}

int ECSJob::submitInterval = 300;
int ECSJob::gahpCallTimeout = 600;
int ECSJob::maxConnectFailures = 3;
int ECSJob::funcRetryInterval = 15;
int ECSJob::maxRetryTimes = 3;

MSC_DISABLE_WARNING(6262) // function uses more than 16k of stack
ECSJob::ECSJob( ClassAd *classad ) :
	BaseJob( classad ),
	...
	updatedOnce( false )
{
	...
	jobAd->LookupString( ATTR_ECS_ACCESS_KEY_ID, m_public_key_file );

	if ( m_public_key_file.empty() ) {
		holdReasonCode = CONDOR_HOLD_CODE_ECSUserError;
		holdReasonSubCode = 1;
	...
	jobAd->LookupString( ATTR_ECS_SECRET_ACCESS_KEY, m_private_key_file );
	...
	jobAd->LookupString( ATTR_ECS_SPOT_PRICE, m_spot_price );
	dprintf( D_FULLDEBUG, "Found ECS spot price: %s\n", m_spot_price.c_str() );

	// look for a spot ID
	jobAd->LookupString( ATTR_ECS_SPOT_REQUEST_ID, m_spot_request_id );
	dprintf( D_FULLDEBUG, "Found ECS spot request ID: %s\n", m_spot_request_id.c_str() );
	...
	jobAd->LookupString( ATTR_ECS_ELASTIC_IP, m_elastic_ip );
	jobAd->LookupString( ATTR_ECS_EBS_VOLUMES, m_ebs_volumes );
	jobAd->LookupString( ATTR_ECS_AVAILABILITY_ZONE, m_availability_zone );
	jobAd->LookupString( ATTR_ECS_VPC_SUBNET, m_vpc_subnet );
	jobAd->LookupString( ATTR_ECS_VPC_IP, m_vpc_ip );
	jobAd->LookupString( ATTR_ECS_BLOCK_DEVICE_MAPPING, m_block_device_mapping );
	jobAd->LookupString( ATTR_ECS_IAM_PROFILE_ARN, m_iam_profile_arn );
	jobAd->LookupString( ATTR_ECS_IAM_PROFILE_NAME, m_iam_profile_name );

	// There's no reason not to build the list of group names here, as well.
	{
		std::string securityIDs;
		m_group_ids = new StringList();
		jobAd->LookupString( ATTR_ECS_SECURITY_IDS, securityIDs );
...

	{
		...
		jobAd->LookupString( ATTR_ECS_PARAM_NAMES, paramNames );
		if( ! paramNames.empty() ) {
			...
			while( (paramName = paramNameList.next()) ) {
				...
				formatstr( jobAdName, "%s_%s", ATTR_ECS_PARAM_PREFIX, jobAdName.c_str() );

				jobAd->LookupString( jobAdName.c_str(), paramValue );
				if( paramValue.empty() ) {
					dprintf( D_ALWAYS, "ECS parameter '%s' had no corresponding value, ignoring.\n", paramName );
					continue;
				}
	...
	jobAd->LookupString( ATTR_ECS_USER_DATA_FILE, m_user_data_file );

	jobAd->LookupString( ATTR_ECS_USER_DATA, m_user_data );
	...
	// will start VM in ECS using m1.small mode.
	jobAd->LookupString( ATTR_ECS_INSTANCE_TYPE, m_instance_type );
	...
	jobAd->LookupString( ATTR_ECS_KEY_PAIR, m_key_pair );
	jobAd->LookupString( ATTR_ECS_KEY_PAIR_FILE, m_key_pair_file );
	...
	if ( !value.empty() ) {
		...
		if ( !token || strcasecmp( token, "ecs" ) ) {
			holdReasonCode = CONDOR_HOLD_CODE_ECSInternalError;
			holdReasonSubCode = 3;
			formatstr( error_string, "%s not of type ecs",
									  ATTR_GRID_RESOURCE );
			goto error_exit;
		}
		...
		if ( token && *token ) {
			...
			holdReasonCode = CONDOR_HOLD_CODE_ECSUserError;
			holdReasonSubCode = 4;
			formatstr( error_string, "%s missing ECS service URL",
									  ATTR_GRID_RESOURCE );
			goto error_exit;
		}
	...
	gahp_path = param( "ECS_GAHP" );
	if ( gahp_path == NULL ) {
		holdReasonCode = CONDOR_HOLD_CODE_ECSAdminError;
		holdReasonSubCode = 6;
		error_string = "ECS_GAHP not defined";
		goto error_exit;
	}

	gahp_log = param( "ECS_GAHP_LOG" );
	if ( gahp_log == NULL ) {
		dprintf(D_ALWAYS, "Warning: No ECS_GAHP_LOG defined\n");
	} else {
		args.AppendArg("-f");
	...
	args.AppendArg("-w");
	gahp_worker_cnt = param_integer( "ECS_GAHP_WORKER_MIN_NUM", 1 );
	args.AppendArg(gahp_worker_cnt);

	args.AppendArg("-m");
	gahp_worker_cnt = param_integer( "ECS_GAHP_WORKER_MAX_NUM", 5 );
	args.AppendArg(gahp_worker_cnt);

	args.AppendArg("-d");
	gahp_debug = param( "ECS_GAHP_DEBUG" );
	if (!gahp_debug) {
	...
	// The ECS GAHP assumes (for purposes of responding to RequestLimitExceeded
	...
	formatstr( gahpName, "ECS-%s@%s", m_public_key_file.c_str(), m_serviceUrl.c_str() );
	// dprintf( D_ALWAYS, "Using %s for GAHP name.\n", gahpName.c_str() );
	gahp = new ECSGahpClient( gahpName.c_str(), gahp_path, &args );
	free(gahp_path);
	...
	myResource =
		ECSResource::FindOrCreateResource(	m_serviceUrl.c_str(),
	...
	if ( !value.empty() ) {
		...
		if ( !token || strcasecmp( token, "ecs" ) ) {
			holdReasonCode = CONDOR_HOLD_CODE_ECSInternalError;
			holdReasonSubCode = 7;
			formatstr( error_string, "%s not of type ecs", ATTR_GRID_JOB_ID );
			goto error_exit;
		}
...
void ECSJob::doEvaluateState()
{
	...
	do {
		...
		{
			case GM_INIT:
				// This is the state all jobs start in when the ECSJob object
				...
				if( ! m_client_token.empty() ) {
					...
					if( ! m_spot_price.empty() ) {
						...
						// isn't baked into the ECS API for spot instances.
			...
			case GM_SAVE_CLIENT_TOKEN: {
				...
				std::string type;
				jobAd->LookupString( ATTR_ECS_SERVER_TYPE, type );
				if ( type != myResource->m_serverType ) {
					jobAd->Assign( ATTR_ECS_SERVER_TYPE, myResource->m_serverType );
					attr_exists = true;
					attr_dirty = true;
				}
			...
			case GM_CREATE_KEY_PAIR: {
				...
				if ( m_should_gen_key_pair )
				{
					...
					rc = gahp->ecs_vm_create_keypair(m_serviceUrl,
									m_public_key_file,
									m_private_key_file,
									m_key_pair,
									m_key_pair_file,
					...
					if ( rc == 0 ||
						...
						dprintf(D_ALWAYS,"(%d.%d) job create keypair failed: %s: %s\n",
								...
								errorString.c_str() );
						gmState = GM_HOLD;
						holdReasonCode = CONDOR_HOLD_CODE_ECSConnectionProblem;
			...
			case GM_START_VM:
				...
				// not incremented after ecs_vm_start? Same for
				// dcloudjob.
				...
				if ( now >= lastSubmitAttempt + submitInterval ) {
					...
					// construct input parameters for ecs_vm_start()
					std::vector< std::string > instance_ids;

					// For a given ECS Job, in its life cycle, the attributes will not change
					...
					// ecs_vm_start() will check the input arguments
					rc = gahp->ecs_vm_start( m_serviceUrl,
											 m_public_key_file,
											 m_private_key_file,
											 m_ami_id,
											 m_key_pair,
					...
					if ( rc != 0 &&
						...
						// the VM has been started successfully in ECS
					...
					if ( rc == 0 ) {
						...
						remoteJobState = ECS_VM_STATE_PENDING;
			...
			case GM_SUBMITTED:
				if( remoteJobState == ECS_VM_STATE_SHUTOFF
				 || remoteJobState == ECS_VM_STATE_STOPPED ) {
				...
				if ( remoteJobState == ECS_VM_STATE_TERMINATED ) {
			...
			case GM_CLEAR_REQUEST: {
				...
				std::string type;
				if ( jobAd->LookupString( ATTR_ECS_SERVER_TYPE, type ) ) {
					jobAd->AssignExpr( ATTR_ECS_SERVER_TYPE, "Undefined" );
				}
			...
			case GM_PROBE_JOB:
				...
				if ( condorState == REMOVED || condorState == HELD ) {
					...
					if( remoteJobState == ECS_VM_STATE_PURGED ) {
						// The instance has been purged, act like we
						// got back 'terminated'
						remoteJobState = ECS_VM_STATE_TERMINATED;
						m_state_reason_code = "purged";
					}

					// We don't check for a status change, because this
					// state is now only entered if we had one.
					if( remoteJobState == ECS_VM_STATE_RUNNING ||
						remoteJobState == ECS_VM_STATE_SHUTTINGDOWN ||
						remoteJobState == ECS_VM_STATE_TERMINATED ) {
						JobRunning();
					...
					if( ! m_state_reason_code.empty() ) {
						...
						jobAd->Assign( ATTR_ECS_STATUS_REASON_CODE, m_state_reason_code.c_str() );
						requestScheddUpdate( this, false );

							//
							// http://docs.amazonwebservices.com/AWSECS/latest/APIReference/ApiReference-ItemType-StateReasonType.html
						...
						if(
						 ...
						 || m_state_reason_code == "Server.SpotInstanceTermination" ) {
							...
							// purged as the ECSStatusReasonCode to the
							// history file.
						} else if(
						 ...
						 || m_state_reason_code == "Client.InvalidSnapshot.NotFound" ) {
							...
							dprintf( D_ALWAYS, "(%d.%d) %s\n", procID.cluster, procID.proc, errorString.c_str() );
							gmState = GM_HOLD;
							holdReasonCode = CONDOR_HOLD_CODE_ECSServerError;
			...
			case GM_CANCEL:
				remoteJobState = ECS_VM_STATE_CANCELLING;
				SetRemoteJobStatus( ECS_VM_STATE_CANCELLING );
				...
				if( m_remoteJobId.empty() ) {
					...
					// because ECS_VM_STATE_CANCELLING isn't be returned by
					// the ECS API.
					probeNow = false;

					rc = gahp->ecs_vm_stop( m_serviceUrl,
											m_public_key_file,
											m_private_key_file,
											m_remoteJobId,
											gahp_error_code);
					...
					if( rc != 0 ) {
						...
						if( strstr( errorString.c_str(), "InstanceNotFound" ) ||
							...
							remoteJobState = ECS_VM_STATE_PURGED;
						} else {
							if( gahp_error_code == "E_CURL_IO" ) { myResource->RequestPing( this ); }
							dprintf( D_ALWAYS, "(%d.%d) job cancel failed: %s: %s\n",
									 ...
									 errorString.c_str() );
							gmState = GM_HOLD;
							holdReasonCode = CONDOR_HOLD_CODE_ECSInstancePotentiallyLostError;
							holdReasonSubCode = 15;
							break;
						}

					// We could save some time here for Amazon users by
					// changing the ECS GAHP to return the (new) state of
			...
			case GM_DELETE:
				...
				if( condorState == REMOVED
					...
					if( m_keypair_created && ! m_key_pair.empty() ) {
						rc = gahp->ecs_vm_destroy_keypair( m_serviceUrl,
						...
						// 'ecs URL remove-keypair', which we'd recognize
			...
			case GM_SPOT_START: {
				...
				rc = gahp->ecs_spot_start(  m_serviceUrl,
											m_public_key_file,
											m_private_key_file,
											m_ami_id,
											m_spot_price,
			...
			case GM_SPOT_CANCEL:
				if( m_spot_request_id.empty() ) {
					...
					// Forcing the state to be something the ECS API doesn't
					// return forces probeNow to set by the next status poll.
					remoteJobState = ECS_VM_STATE_SPOT_CANCELLING;
					SetRemoteJobStatus( ECS_VM_STATE_SPOT_CANCELLING );

					// Send a command to the GAHP, or poll for its result(s).
					rc = gahp->ecs_spot_stop(   m_serviceUrl,
												m_public_key_file,
												m_private_key_file,
												m_spot_request_id,
												gahp_error_code );
			...
			case GM_SPOT_QUERY: {
				...
				if( remoteJobState == ECS_VM_STATE_PURGED ) {
					...
					// spot request ID and when the ECS service includes that
				...
				if( remoteJobState == "open" ) {
					...
					// remote job state to ECS_VM_STATE_PENDING.  Since the
...
BaseResource* ECSJob::GetResource()
{
	return (BaseResource *)myResource;
}


// setup the public name of ecs remote VM, which can be used the clients
void ECSJob::SetRemoteVMName( const char * newName )
{
	if( newName == NULL ) {
		newName = "Undefined";
	}
	...
	std::string oldName;
	jobAd->LookupString( ATTR_ECS_REMOTE_VM_NAME, oldName );
	if( oldName != newName ) {
		jobAd->Assign( ATTR_ECS_REMOTE_VM_NAME, newName );
		requestScheddUpdate( this, false );
	}
}

void ECSJob::SetKeypairId( const char *keypair_id )
{
	if ( keypair_id == NULL ) {
		m_key_pair = "";
	} else {
	...
	jobAd->Assign( ATTR_ECS_KEY_PAIR, m_key_pair );

	requestScheddUpdate( this, false );
}

void ECSJob::SetClientToken(const char *client_token)
{
	...
	ECSSetRemoteJobId(m_client_token.empty() ? NULL : m_client_token.c_str(),
				   m_remoteJobId.c_str());
}

void ECSJob::SetInstanceId( const char *instance_id )
{
	...
	if( instance_id == NULL ) {
		...
		jobAd->Assign( ATTR_ECS_INSTANCE_NAME, m_remoteJobId );
	}
	ECSSetRemoteJobId( m_client_token.c_str(),
					m_remoteJobId.empty() ? NULL : m_remoteJobId.c_str() );
}

// ECSSetRemoteJobId() is used to set the value of global variable "remoteJobID"
// Don't call this function directly!
// It doesn't update m_client_token or m_remoteJobId!
// Use SetClientToken() or SetInstanceId() instead.
void ECSJob::ECSSetRemoteJobId( const char *client_token, const char *instance_id )
{
	string full_job_id;
	if ( client_token && client_token[0] ) {
		formatstr( full_job_id, "ecs %s %s", m_serviceUrl.c_str(), client_token );
...
string ECSJob::build_ami_id()
{
	string ami_id;
	char* buffer = NULL;

	if ( jobAd->LookupString( ATTR_ECS_AMI_ID, &buffer ) ) {
		ami_id = buffer;
		free (buffer);
	}
	return ami_id;
...
// http://docs.amazonwebservices.com/AWSECS/latest/UserGuide/Run_Instance_Idempotency.html
string ECSJob::build_client_token()
{
#ifdef WIN32
	GUID guid;
...
std::string ECSJob::build_keypair()
{
	...
	// Some ECS implementations (OpenStack) restrict the keypair name to
...
StringList* ECSJob::build_groupnames()
{
	...
	// 1. clients assign ATTR_ECS_SECURITY_GROUPS in condor_submit file, then we will use those
	//    security group names.
	// 2. clients don't assign ATTR_ECS_SECURITY_GROUPS in condor_submit file, then we will use
	//    the default security group (by just keeping group_names is empty).

	if ( jobAd->LookupString( ATTR_ECS_SECURITY_GROUPS, &buffer ) ) {
...
bool ECSJob::remove_keypair_file(const char* filename)
{
	if (filename == NULL) {
...
void ECSJob::print_error_code( const char* error_code,
...
void ECSJob::associate_n_attach()
{
	...
	char *buffer = NULL;
	if (jobAd->LookupString(ATTR_ECS_TAG_NAMES, &buffer)) {
		StringList tags;
		...
		while ((tagName = tagNames.next())) {
			...
			string tag;
			string tagAttr(ATTR_ECS_TAG_PREFIX);
		...
		rc = gahp->ecs_create_tags(m_serviceUrl.c_str(),
								   m_public_key_file.c_str(),
								   m_private_key_file.c_str(),
								   m_remoteJobId.c_str(),
								   tags,
		...
		default:
			if( gahp_error_code == "E_CURL_IO" ) { myResource->RequestPing( this ); }
			dprintf(D_ALWAYS,
					"Failed ecs_create_tags returned %s continuing w/job\n",
					gahp_error_code.c_str());
			break;
		}
		gahp_error_code = "";
	...
	{
		rc = gahp->ecs_associate_address(m_serviceUrl,
										 m_public_key_file,
										 m_private_key_file,
										 m_remoteJobId,
										 m_elastic_ip,
		...
		switch (rc)
		{
			...
			default:
				if( gahp_error_code == "E_CURL_IO" ) { myResource->RequestPing( this ); }
				dprintf(D_ALWAYS,
						"Failed ecs_associate_address returned %s continuing w/job\n",
						gahp_error_code.c_str());
				break;
		}
		gahp_error_code = "";
	...
	if (!m_ebs_volumes.empty())
	{
		...
		while( bcontinue && (volume_str = vols.next() ) != NULL )
		{
			...
			rc = gahp->ecs_attach_volume(m_serviceUrl,
										 m_public_key_file,
										 m_private_key_file,
										 volume_id,
										 m_remoteJobId,
			...
			switch (rc)
			{
				...
				default:
					...
					bcontinue=false;
					dprintf(D_ALWAYS,
							"Failed ecs_attach_volume returned %s continuing w/job\n",
							gahp_error_code.c_str());
					break;
			}
			gahp_error_code = "";
...
void ECSJob::StatusUpdate( const char * instanceID,
						   const char * status,
...
void ECSJob::SetRequestID( const char * requestID ) {
	if( requestID == NULL ) {
		if( ! m_spot_request_id.empty() ) {
		...
		jobAd->AssignExpr( ATTR_ECS_SPOT_REQUEST_ID, "Undefined" );
		m_spot_request_id = std::string();
	} else {
		jobAd->Assign( ATTR_ECS_SPOT_REQUEST_ID, requestID );
...
// command fails are ecs_vm_destroy_keypair() and ecs_vm_start(). For
...
void ECSJob::ResourceLeaseExpired() {
...
void ECSJob::NotifyResourceDown() {
	BaseJob::NotifyResourceDown();

	if( resourceLeaseTID != -1 ) { return; }

	time_t now = time( NULL );
	int leaseDuration = param_integer( "ECS_RESOURCE_TIMEOUT", -1 );
	if( leaseDuration == -1 ) {
		return;
	}
	...
	if( leaseDuration > 0 ) {
		resourceLeaseTID = daemonCore->Register_Timer( leaseDuration,
			(TimerHandlercpp) & ECSJob::ResourceLeaseExpired,
