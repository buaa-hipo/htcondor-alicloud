...
#include "ecsresource.h"
#include "gridmanager.h"

#define HASH_TABLE_SIZE	500

HashTable <HashKey, ECSResource *>
	ECSResource::ResourcesByName( HASH_TABLE_SIZE, hashFunction );

const char * ECSResource::HashName( const char * resource_name,
		const char * public_key_file, const char * private_key_file )
{
	static std::string hash_name;
	formatstr( hash_name, "ecs %s#%s#%s", resource_name, public_key_file, private_key_file );
	return hash_name.c_str();
}


ECSResource* ECSResource::FindOrCreateResource(const char * resource_name,
	const char * public_key_file, const char * private_key_file )
{
	int rc;
	...
	if ( rc != 0 ) {
		resource = new ECSResource( resource_name, public_key_file, private_key_file );
		ASSERT(resource);
		resource->Reconfig();
...
ECSResource::ECSResource( const char *resource_name,
	const char * public_key_file, const char * private_key_file ) :
		...
		m_checkSpotNext( false )
{
	...
	char * gahp_path = param( "ECS_GAHP" );
	if ( gahp_path == NULL ) {
		dprintf(D_ALWAYS, "ECS_GAHP not defined! \n");
		return;
	}
	...
	formatstr( gahp_name, "ECS-%s@%s", m_public_key_file, resource_name );
	// dprintf( D_ALWAYS, "Using %s for GAHP name.\n", gahp_name.c_str() );
	gahp = new ECSGahpClient( gahp_name.c_str(), gahp_path, &args );
	gahp->setNotificationTimerId( pingTimerId );
	gahp->setMode( GahpClient::normal );
	gahp->setTimeout( ECSJob::gahpCallTimeout );

	status_gahp = new ECSGahpClient( gahp_name.c_str(), gahp_path, &args );

	StartBatchStatusTimer();

    status_gahp->setNotificationTimerId( BatchPollTid() );
	status_gahp->setMode( GahpClient::normal );
	status_gahp->setTimeout( ECSJob::gahpCallTimeout );
...
const char *ECSResource::ResourceType()
{
	return "ecs";
}

const char *ECSResource::GetHashName()
{
	return HashName( resourceName, m_public_key_file, m_private_key_file );
}

void ECSResource::PublishResourceAd( ClassAd *resource_ad )
{
	BaseResource::PublishResourceAd( resource_ad );

	resource_ad->Assign( ATTR_ECS_ACCESS_KEY_ID, m_public_key_file );
	resource_ad->Assign( ATTR_ECS_SECRET_ACCESS_KEY, m_private_key_file );
...
// we will use ecs command "status_all" to do the Ping work
void ECSResource::DoPing( unsigned& ping_delay, bool& ping_complete, bool& ping_succeeded )
{
	// Since ECS doesn't use proxy, we should use Startup() to replace isInitialized()
	...
	int rc = gahp->ecs_vm_server_type( resourceName, m_public_key_file, m_private_key_file, m_serverType, error_code );
...
ECSResource::BatchStatusResult ECSResource::StartBatchStatus() {
    ASSERT( status_gahp );

    // First, fetch the GAHP's statistics.
    StringList statistics;
    int rc = status_gahp->ecs_gahp_statistics( statistics );
    ...
    if( ! m_checkSpotNext ) {
        ...
        int rc = status_gahp->ecs_vm_status_all( resourceName,
        ...
        if( rc != 0 ) {
            std::string errorString = status_gahp->getErrorString();
            dprintf( D_ALWAYS, "Error doing batched ECS status query: %s: %s.\n",
		...
		while ( (nextBaseJob = registeredJobs.Next()) ) {
			nextJob = dynamic_cast< ECSJob * >( nextBaseJob );
			ASSERT( nextJob );
        ...
        for( int i = 0; i < returnStatus.number(); i += 7 ) {
            ...
            if( ! clientToken.empty() && clientToken != "NULL" ) {
                ...
                formatstr( remoteJobID, "ecs %s %s", resourceName, clientToken.c_str() );
                ...
                if( rc == 0 ) {
                    ASSERT( tmp );
                    ECSJob * job = dynamic_cast< ECSJob * >( tmp );
                    if( job == NULL ) {
                        EXCEPT( "Found non-ECSJob identified by '%s'.", remoteJobID.c_str() );
                    }
    ...
    if( m_checkSpotNext ) {
        ...
        int spotRC = status_gahp->ecs_spot_status_all( resourceName,
                        m_public_key_file, m_private_key_file,
        ...
        if( spotRC != 0 ) {
            std::string errorString = status_gahp->getErrorString();
            dprintf( D_ALWAYS, "Error doing batched ECS spot status query: %s: %s.\n",
                     spotErrorCode.c_str(), errorString.c_str() );
            return BSR_ERROR;
        }

        List<ECSJob> mySpotJobs;
        ECSJob * nextSpotJob = NULL;
...
ECSResource::BatchStatusResult ECSResource::FinishBatchStatus() {
    return StartBatchStatus();
}

bool ECSResource::ServerTypeQueried( ECSJob *job ) {
	if ( !m_serverType.empty() ) {
		return true;
	}
	std::string type;
	if ( job && job->jobAd->LookupString( ATTR_ECS_SERVER_TYPE, type ) ) {
		return true;
	}
	return false;
}

bool ECSResource::ClientTokenWorks( ECSJob *job )
{
	...
	if ( m_serverType.empty() && job &&
		 job->jobAd->LookupString( ATTR_ECS_SERVER_TYPE, type ) ) {
...
bool ECSResource::ShuttingDownTrusted( ECSJob *job ) {
...
#define SAMPLES 4
#define QUANTUM 1
ECSResource::GahpStatistics::GahpStatistics() :
...
void ECSResource::GahpStatistics::Init() {
...
void ECSResource::GahpStatistics::Clear() {
	pool.Clear();
...
void ECSResource::GahpStatistics::Advance() {
	pool.Advance( QUANTUM );
}

void ECSResource::GahpStatistics::Publish( ClassAd & ad ) const {
	pool.Publish( ad, (IF_BASICPUB & IF_PUBLEVEL) | IF_RECENTPUB );
}

void ECSResource::GahpStatistics::Unpublish( ClassAd & ad ) const {
	pool.Unpublish( ad );
}
