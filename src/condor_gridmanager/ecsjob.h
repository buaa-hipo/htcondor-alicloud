...
class ECSJob : public BaseJob
{
public:

	ECSJob( ClassAd *classad );
	~ECSJob();
	...
	void ECSSetRemoteJobId( const char *client_token, const char *instance_id );
	...
	//  38: internal error.  Example: the grid resource type was not "ecs";
	//      the GAHP should never have seen this job ad.
	//  39: administrator error.  Example: ECS_GAHP being unset.
	...
	// to debugged; CODE 42 REQUIRES THE ECS ACCOUNT OWNER TO INVESTIGATE.
	//
	int holdReasonCode;
