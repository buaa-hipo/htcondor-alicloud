...
void registerTestUsers() {
    ...
    // The Alicloud ECS ID we actually use for testing.
...
User & validateAndAcquireUser( AttributeValueMap & avm, std::string & userID, std::string & reply, bool & found ) {
    ...
    userID = getObject< std::string >( avm, "AccessKeyId", found );
    if( (! found) || userID.empty() ) {
        fprintf( stderr, "Failed to find user ID in query.\n" );
        reply = "Required parameter 'AccessKeyId' missing or empty.\n";
        found = false;
        return dummyUser;
    }
    ...
    if( ! found ) {
        std::ostringstream os;
        os << "Unknown AccessKeyId '" << userID << "'." << std::endl;
        reply = os.str();
...
bool handleRunInstances( AttributeValueMap & avm, std::string & reply, unsigned requestNumber ) {
    ...
    // Validate the RegionID, ImageId parameters, as well
    // as the optional parameters Amount KeyName, InstanceType, SecurityGroup*,
    // and UserData.

    // We presently assume all imageIDs are valid.
    
    std::string RegionID = getObject< std::string >( avm, "RegionId", found );
    if( (! found) || RegionID.empty() ) {
        fprintf( stderr, "Failed to find RegionId in query.\n" );
        reply = "Required parameter RegionId missing or empty.\n";
        return false;
    }
    ...
    std::string Amount = getObject< std::string >( avm, "Amount", found );
    if((! found) || Amount.empty()) {
        Amount = "1";
    }


    std::string keyName = getObject< std::string >( avm, "KeyPairName", found );
    if( ! keyName.empty() ) {
    ...
    xml << "<RunInstancesResponse xmlns=\"weiyi test\">" << std::endl;
    ...
    xml << "<Amount>" << Amount << "</Amount>" << std::endl;
    xml << "<RegionId>" << RegionID << "</RegionId>" << std::endl;
...
bool handleTerminateInstances( AttributeValueMap & avm, std::string & reply, unsigned requestNumber ) {
    ...
    std::string instanceID = getObject< std::string >( avm, "InstanceId", found );
    if( (! found) || instanceID.empty() ) {
        fprintf( stderr, "Failed to find instanceID in query.\n" );
        reply = "Required parameter InstanceId missing or empty.\n";
        return false;
    }
    ...
    // Construct the XML reply.
    std::ostringstream xml;
    xml << "<DeleteInstancesResponse xmlns=\"weiyi test\">" << std::endl;
...
bool handleDescribeInstances( AttributeValueMap & avm, std::string & reply, unsigned requestNumber ) {
    ...
    std::ostringstream xml;
    xml << "<DescribeInstancesResponse xmlns=\"weiyi test\">" << std::endl;
...
bool handleCreateTags( AttributeValueMap & avm, std::string & reply, unsigned requestNumber ) {
    ...
    std::string tagName = getObject< std::string >( avm, "Tag.1.Key", found );
    ...
    std::string tagValue = getObject< std::string >( avm, "Tag.1.Value", found );
    ...
    std::ostringstream xml;
    xml << "<CreateTagsResponse xmlns=\"weiyi test\">" << std::endl;
    xml << xmlTag( "requestId", rID ) << std::endl;
...
bool handleCreateKeyPair( AttributeValueMap & avm, std::string & reply, unsigned requestNumber ) {
    ...
    if( (! found) || keyName.empty() ) {
        fprintf( stderr, "Failed to find KeyPairName in query.\n" );
        reply = "Required parameter KeyPairName missing or empty.\n";
        return false;
    }
    ...
    std::ostringstream xml;
    xml << "<CreateKeyPairResponse xmlns=\"weiyi test\">" << std::endl;
    xml << xmlTag( "requestId", rID ) << std::endl;
    xml << xmlTag( "keyPairName", kp.keyName ) << std::endl;
...
bool handleDeleteKeyPair( AttributeValueMap & avm, std::string & reply, unsigned requestNumber ) {
    ...
    std::string keyName = getObject< std::string >( avm, "KeyPairNames", found );
    if( (! found) || keyName.empty() ) {
        fprintf( stderr, "Failed to find KeyPairName in query.\n" );
        reply = "Required parameter KeyPairName missing or empty.\n";
        return false;
    ...
    if( found ) {
        ...
        // Alicloud's ECS blithely succeeds when this happen, but we'll
    ...
    std::ostringstream xml;
    xml << "<DeleteKeyPairsResponse xmlns=\"weiyi test\">" << std::endl;
...
bool handleDescribeKeyPairs( AttributeValueMap & avm, std::string & reply, unsigned requestNumber ) {
    ...
    xml << "<DescribeKeyPairsResponse xmlns=\"weiyi test\">" << std::endl;
    xml << "<keySet>" << std::endl;
...
void registerAllHandlers() {
    ...
    simulatorActions[ "DeleteInstance" ] = & handleTerminateInstances;
    ...
    simulatorActions[ "ADDTags" ] = & handleCreateTags;
}
...
/*
 * The most fragile part of the ECS GAHP is the query signing, so
 * we'd like to validate it.  See the comments in alicloudCommands.cpp
 * for more details.
 *
...
int printLeakSummary() {
    ...
    for( ; u != users.end(); ++u ) {
        ...
        // We never garbage-collect terminated instances (since ECS leaves
