...
#ifndef __ALICLOUDGAHP_COMMON_H__
#define __ALICLOUDGAHP_COMMON_H__
...
#define ALICLOUD_SCRIPT_INTERPRETER "perl"
#define ALICLOUD_SCRIPT_NAME "condor_alicloud.pl"

#define ALICLOUD_COMMAND_SUCCESS_OUTPUT	"0"
#define ALICLOUD_COMMAND_ERROR_OUTPUT		"1"

#define ALICLOUD_HTTP_PROXY   "ALICLOUD_HTTP_PROXY"

typedef bool (*ioCheckfn)(char **argv, int argc);
typedef bool (*workerfn)(char **argv, int argc, std::string &output_string);

class AlicloudGahpCommand 
{
	public:
		AlicloudGahpCommand(const char*, ioCheckfn, workerfn);
		std::string command;
...
int numofAlicloudCommands(void);
int allAlicloudCommands(StringList &output);
